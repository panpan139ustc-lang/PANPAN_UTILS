...
static const unsigned DEBUG_NEW_MAGIC = 0x4442474E; // 魔数

static const int ALIGNED_LIST_ITEM_SIZE = ALIGN(sizeof(new_ptr_list_t));

static new_ptr_list_t new_ptr_list = { &new_ptr_list, &new_ptr_list, 0, {""}, 0, 0, DEBUG_NEW_MAGIC };//双链表头节点

static std::mutex new_ptr_lock; // 实现多线程串行访问链表

static std::mutex new_output_lock; // 实现多线程串行往控制台打印信息

static std::size_t total_mem_alloc = 0;

bool new_autocheck_flag = true; //自动检查开关

bool new_verbose_flag = false; //用来实时检测
...
static void free_pointer(void* usr_ptr, void* addr, bool is_array) {
	...
	if (ptr->magic != DEBUG_NEW_MAGIC) { //检查内存块的元数据(数组越界)是否被修改或非法的指针(栈内存或已释放的指针)
		{
			std::unique_lock<std::mutex> lock(new_output_lock);
